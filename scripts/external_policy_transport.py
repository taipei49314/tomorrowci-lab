...
def _url_template(value: object, label: str) -> str:
    ...
    if sorted(fields) != ["candidate_commit", "candidate_manifest_sha256_hex"]:
        raise ValueError(
            f"{label} must contain each candidate identity field exactly once"
        )
    if (
        _TEMPLATE.sub("", value).find("{") >= 0
...
def load_transport(
    config: Path, allowed_signers: Path
) -> tuple[dict, authorization._Snapshot, authorization._Snapshot]:
    ...
    authorization._object(
        value,
        {"kind", "schema_version", "transport", "trust"},
        "external policy transport",
    )
    if value["kind"] != KIND or value["schema_version"] != 1:
        raise ValueError("external policy transport identity mismatch")
    trust = authorization._object(
        value["trust"],
        {"allowed_signers_sha256", "auditor_principal", "namespace"},
        "external policy transport trust",
    )
    if trust["namespace"] != authorization.NAMESPACE:
        raise ValueError("external policy transport namespace mismatch")
    if type(trust["auditor_principal"]) is not str or not TOKEN.fullmatch(
        trust["auditor_principal"]
    ):
        raise ValueError("external policy transport auditor principal is invalid")
    if type(trust["allowed_signers_sha256"]) is not str or not SHA256.fullmatch(
    ...
    authorization._validate_allowed_signers(
        allowed_snapshot, trust["auditor_principal"], trust["allowed_signers_sha256"]
    )
    transport = authorization._object(
        value["transport"],
        {"maximum_bytes", "policy_url_template", "signature_url_template"},
        "external policy transport endpoint",
    )
    if (
        type(transport["maximum_bytes"]) is not int
        or not 1 <= transport["maximum_bytes"] <= MAX_BYTES
    ):
        raise ValueError("external policy transport maximum bytes is invalid")
    policy_template = _url_template(
        transport["policy_url_template"], "policy URL template"
    )
    signature_template = _url_template(
        transport["signature_url_template"], "policy signature URL template"
    )
...
def _candidate_identity(candidate_manifest: Path) -> tuple[str, str, str]:
    ...
    if type(source["repository"]) is not str or not authorization.SLUG.fullmatch(
        source["repository"]
    ):
...
def render_urls(
    config: dict, *, candidate_commit: str, candidate_manifest_sha256: str
) -> tuple[str, str]:
    if not SHA.fullmatch(candidate_commit) or not SHA256.fullmatch(
        candidate_manifest_sha256
    ):
        raise ValueError("candidate identity is malformed")
    values = {
        "candidate_commit": candidate_commit,
        "candidate_manifest_sha256_hex": candidate_manifest_sha256.removeprefix(
            "sha256:"
        ),
    }
...
def _fetch(url: str, *, maximum_bytes: int, opener: object | None = None) -> bytes:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "Pragma": "no-cache",
            "User-Agent": "TomorrowCI-external-policy-transport/1",
        },
    )
    try:
        with active_opener.open(request, timeout=30) as response:  # type: ignore[union-attr]
            ...
            if response.headers.get("Content-Encoding") not in (None, "identity"):
                raise ValueError(
                    "external policy transport content encoding is forbidden"
                )
...
def fetch_policy(
    *,
    ...
    transport, config_snapshot, allowed_snapshot = load_transport(
        config, allowed_signers
    )
    repository, commit, manifest_sha256 = _candidate_identity(candidate_manifest)
    policy_url, signature_url = render_urls(
        transport, candidate_commit=commit, candidate_manifest_sha256=manifest_sha256
    )
    ...
    policy_snapshot = authorization._Snapshot(
        "externally transported authorization policy",
        output_policy.absolute(),
        policy_data,
        _sha256(policy_data),
    )
    signature_snapshot = authorization._Snapshot(
        "externally transported policy signature",
        output_signature.absolute(),
        signature_data,
        _sha256(signature_data),
    )
    policy = authorization._load_policy(policy_snapshot)
    expected_candidate = policy["candidate"]
    if (
        expected_candidate["repository"] != repository
        or expected_candidate["commit"] != commit
        or expected_candidate["manifest_sha256"] != manifest_sha256
    ):
    ...
    return {
        ...
        "kind": RECEIPT_KIND,
        "policy": {"sha256": policy_snapshot.sha256, "url": policy_url},
        "signature": {"sha256": signature_snapshot.sha256, "url": signature_url},
        "status": "FETCHED_FROM_EXTERNAL_SIGNED_TRANSPORT_ONLY",
        "trust": trust,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        receipt = fetch_policy(
            **{key: value for key, value in vars(args).items() if key != "fetch"}
        )
